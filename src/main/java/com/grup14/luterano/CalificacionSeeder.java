...
public class CalificacionSeeder implements CommandLineRunner {
                    ...
                    for (MateriaCurso materiaCurso : materiasCurso) {
                        ...
                        for (int etapa = 1; etapa <= ETAPAS; etapa++) {
                            for (int numeroNota = 1; numeroNota <= NOTAS_POR_ETAPA; numeroNota++) {
                                ...
                                // Generar nota con ~90% de probabilidad de estar aprobada (>= 6)
                                int notaAleatoria = generarNotaAprobada90();
    ...
    /**
     * Genera una nota entre 1 y 10 donde aproximadamente el 90% de las veces
     * la nota es aprobada (>= 6).
     */
    private static int generarNotaAprobada90() {
        double p = RANDOM.nextDouble(); // [0.0, 1.0)

        if (p < 0.9) {
            // 90%: nota aprobada, entre 6 y 10 (inclusive)
            return RANDOM.nextInt(6, 11); // 6,7,8,9,10
        } else {
            // 10%: nota desaprobada, entre 1 y 5 (inclusive)
            return RANDOM.nextInt(1, 6); // 1,2,3,4,5
        }
    }
}